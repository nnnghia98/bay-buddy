...
export function FormulaPreview({
  ...
  return (
    <div className="grid gap-3 rounded-lg border border-border bg-secondary/20 p-3">
      ...
      {preview.errors?.map((error) => <p className="text-xs font-medium text-destructive" key={`${error.code}:${error.message}`}>{formulaMessage(t, error.code)}</p>)}
      {preview.warnings?.map((warning) => <p className="text-xs text-amber-700" key={`${warning.code}:${warning.message}`}>{formulaMessage(t, warning.code)}</p>)}
          ...
          {preview.results.map((row) => (
            <div className="grid grid-cols-[5rem_1fr] border-t border-border px-3 py-2 text-sm" key={row.row_number}>
              ...
              {row.error_code
                ...
                : <span className="font-medium tabular-nums">{row.value == null ? t("workbookEditor.editor.columns.preview.blank") : new Intl.NumberFormat().format(row.value)}</span>}
            </div>
          ))}
        </div>
      ) : null}
